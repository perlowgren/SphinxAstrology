...
double calendar_get_julian_day(int y,int m,double d,int t) {
    if(m<3) y = y-1,m = m+12;
    long a = 0,b = 0;
    if(t==GREGORIAN) a = INT(y/100.0),b = 2-a+INT(a/4.0);
    return INT(365.25*(y+4716))+INT(30.6001*(m+1))+d+b-1524.5;
}

int calendar_is_leap(int y,int t) {
    return t==GREGORIAN? ((y%100)==0? ((y%400)==0) : ((y%4)==0)) : ((y%4)==0);
}
...
void calendar_set(calendar *c,int y,int m,int d,int h,int n,double s,int t) {
    ...
    c->minute = n;
    c->second = s;
    c->jd = calendar_get_julian_day(y,m,d+(h/24.0)+(n/1440.0)+(s/86400.0),t);
    c->type = t;
    adjust(c);
}
...
int calendar_days_in_month(calendar *c) {
    return c->month==2 && calendar_is_leap(c->year,c->type)? month_length[1]+1 : month_length[c->month-1];
}

int calendar_days_in_year(calendar *c) {
    return calendar_is_leap(c->year,c->type)? 366 : 365;
}

double calendar_day_of_year(calendar *c) {
    return c->jd-calendar_get_julian_day(c->year,1,1,c->type)+1;
}

double calendar_fractional_year(calendar *c) {
    long d = calendar_days_in_year(c);
    return c->year+((c->jd-calendar_get_julian_day(c->year,1,1,c->type))/d);
}
